...
\startquestion[points = 2]
  设集合 $A = \{x \mid -1 < x < 4\}$，$B = \{2, 3, 4, 5\}$，则 $A \cap B = $ \fillin%[A]
...
%\makepapertitle[\currentpapertitle]
...
\setupmaterial[title=,author=,source=]
\setcounter[number][0]
\startmaterial
...
\setupmaterial [number][numberconversion=A,before=,after=,way=bychapter]
\setcounter[number][0]
