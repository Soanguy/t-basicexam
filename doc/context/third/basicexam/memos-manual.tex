...
\def\showpaletcolors{%
  ...
  \HL
  \NC 颜色名称 \NC 示例 \NC 颜色名称 \NC 示例 \NC\NR
  \HL
...
\startitemize[packed]
\item 纸张大小和页面布局：\goto{第五章 页面布局}[page:layout]
\item 字体设置：\goto{第四章 字体系统}[page:font]
\item 主题颜色：\goto{第六章 颜色系统}[page:color]
\item 章节标题样式：\goto{第七章 章节标题}[page:chapter]
\item 页眉页脚样式：\goto{第八章 页眉页脚}[page:headerfooter]
\item 目录样式：\goto{第九章 目录系统}[page:toc]
\stopitemize
...
模块预定义了 11 种基础颜色，每种颜色都有 7 个层次（pale/soft/light/normal/dark/deep/black）：
...
每种主题颜色都有 7 个层级变体，形成完整的颜色体系：

\starttabulate[|l|p|]
\HL
\NC 颜色变体 \NC 说明 \NC\NR
\HL
\NC palethemecolor \NC 极浅主题色 \NC\NR
...
\NC blackthemecolor \NC 最深主题色 \NC\NR
\HL
...
\NC toccolor \NC 目录颜色 \NC\NR
\NC referencecolor \NC 引用颜色 \NC\NR
\NC optcolor \NC 选项颜色 \NC\NR
\NC lightoptcolor \NC 浅色选项颜色 \NC\NR
...
拼音注音功能为汉字自动或手动添加拼音标注，适用于教育材料、语言学习文档等场景。
...
使用 \type{startpinyinscope} 环境自动为其中的汉字添加拼音：
...
启用 \type{markpolyphone=yes} 可以标记多音字：
...
使用 \type{\setuppinyin} 设置拼音参数：

\starttabulate[|l|l|p(8cm)|]
\HL
\NC 参数 \NC 默认值 \NC 说明 \NC\NR
\HL
\NC vsep \NC 1em \NC 拼音与汉字的垂直间距 \NC\NR
\NC hsep \NC 0pt \NC 拼音字符间的水平间距 \NC\NR
\NC pysep \NC \tex{spaceskip} \NC 拼音音节间的间距 \NC\NR
\NC ratio \NC 4 \NC 拼音字号比例（相对于汉字） \NC\NR
\NC multiple \NC no \NC 是否显示多音字选项 \NC\NR
\NC markpolyphone \NC no \NC 是否标记多音字 \NC\NR
\HL
\stoptabulate
...
\startbuffer
% 调整拼音大小
\xpinyin[ratio=6]{汉}  % 更大
\xpinyin[ratio=3]{汉}  % 更小
...
笔画显示功能用于展示汉字的笔画顺序和结构，适用于汉字教学、书法练习等场景。
...
\NC tian \NC 田字格（十字线） \NC\NR
\NC outertian \NC 外框 + 田字格 \NC\NR
\NC mi \NC 米字格（十字+对角线） \NC\NR
...
\NC 参数 \NC 默认值 \NC 说明 \NC\NR
\HL
\NC width \NC 1cm \NC 绘图宽度 \NC\NR
\NC frame \NC outer \NC 边框类型 \NC\NR
\NC natural \NC no \NC 是否使用自然宽度 \NC\NR
\NC framestroke \NC black \NC 边框线条颜色 \NC\NR
\NC framefill \NC (无) \NC 边框填充颜色 \NC\NR
\NC framelinewidth \NC 0.5pt \NC 边框线条宽度 \NC\NR
\NC framedashpattern \NC solid \NC 边框线条样式（solid/dashed/dotted） \NC\NR
\NC iframestroke \NC 0.5white \NC 内框线条颜色 \NC\NR
\NC iframelinewidth \NC 0.3pt \NC 内框线条宽度 \NC\NR
\NC iframedashpattern \NC dashed \NC 内框线条样式 \NC\NR
\NC charstroke \NC black \NC 字符线条颜色 \NC\NR
\NC charfill \NC black \NC 字符填充颜色 \NC\NR
\NC charlinewidth \NC 0.3pt \NC 字符线条宽度 \NC\NR
\NC chardashpattern \NC solid \NC 字符线条样式 \NC\NR
\NC laststroke \NC red \NC 最后一笔线条颜色 \NC\NR
\NC lastfill \NC red \NC 最后一笔填充颜色 \NC\NR
\NC lastlinewidth \NC 0.3pt \NC 最后一笔线条宽度 \NC\NR
\NC lastdashpattern \NC solid \NC 最后一笔线条样式 \NC\NR
\NC innert \NC 0pt \NC 上内边距 \NC\NR
\NC innerb \NC 0pt \NC 下内边距 \NC\NR
\NC innerl \NC 0pt \NC 左内边距 \NC\NR
\NC innerr \NC 0pt \NC 右内边距 \NC\NR
\NC inner \NC (无) \NC 统一设置内边距 \NC\NR
\NC spacing \NC 0em \NC 字符间距（仅 \type{\bihuashowall}） \NC\NR
\HL
\stoptabulate
