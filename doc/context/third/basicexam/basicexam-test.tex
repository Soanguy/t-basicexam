% basicexam-test.tex
% Comprehensive documentation and test suite for the basicexam module
% Author: Shueng Mosan
% Date: 2024-10-24
\usemodule[memos]
\usemodule[basicexam]
...
By default, answers are extracted from chapter 0 (i.e., no chapter). Therefore, if answers are located under chapters, you should start from chapter 1.

\startbuffer
\dorecurse{5}{\typeanswerbychap{1},}

\newcount\tempa\tempa=2\relax
\dorecurse{5}{\typeanswerbychap[\tempa]{1},\advance\tempa by 1\relax}
...
\startbuffer
\typeanswertable[2,5]          \null\par
\typeanswertable[2,5][1]       \null\par
\typeanswertable[2,5][1][6]    \null\par

\typeanswertablealt[2,5]       \null\par
\typeanswertablealt[2,5][1]    \null\par
\typeanswertablealt[2,5][1][6] \null\par
\stopbuffer
...
\section{User Question Environment}

The \type{userquestion} and \type{useritem} environments provide flexibility
for question types not covered by the standard environments.

\startbuffer
\setupuserquestion[point=1,showanswer=true,showpoint=true]
\useritem{Answer 1} XXXX

\useritem{Answer 2} XXXX

\useritem{Answer 3} XXXX

\startuserquestion
XXXXXX\useritem{A} \useritem{B} \useritem{C}
\stopuserquestion
\stopbuffer
...
The module provides multilingual support through \type{\userlanguage}:

\starttabulate[|l|l|p|]
\HL
\NC Language \NC Code \NC Description \NC\NR
\HL
\NC English \NC en \NC Latin script, modern font \NC\NR
\NC Chinese (Simplified) \NC cn, hans \NC Simplified Chinese \NC\NR
\NC Chinese (Traditional) \NC hant \NC Traditional Chinese \NC\NR
\NC Japanese \NC ja \NC Japanese with ruby support \NC\NR
\HL
\stoptabulate

\startbuffer
\userlanguage[en]{This is English text.}

\userlanguage[cn]{这是中文文本。}

\userlanguage[ja]{これは日本語のテキストです。}
\stopbuffer
