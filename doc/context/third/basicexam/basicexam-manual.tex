...
\definechoice[mychoice][
  option={text,markcircle,packed,joinedup},
  inlinechoice=true,
...
\starttabulate[|l|p(8cm)|l|]
\HL
\NC 属性 \NC 说明 \NC 获取方式 \NC\NR
\HL
\NC id \NC 节点唯一标识（如 \type{jobname-1-Q-1}） \NC \type{\UnlimitedCurrentID} \NC\NR
\NC order \NC 层级顺序（如 \type{1}, \type{2-1}, \type{2-2-1}） \NC \type{\UnlimitedGetOrder{ID}} \NC\NR
\NC answer \NC 答案内容 \NC \type{\UnlimitedDisplayAnswer{ID}} \NC\NR
\NC point \NC 当前节点分数（叶子节点有效） \NC \type{\UnlimitedDisplayPoint{ID}} \NC\NR
\NC totalpoint \NC 该节点及子节点总分 \NC \type{\UnlimitedTotalPoint{ID}} \NC\NR
\NC totalnumber \NC 该节点及子节点总数 \NC \type{\UnlimitedTotalNumber{ID}} \NC\NR
\NC is_leaf \NC 是否为叶子节点 \NC \type{\UnlimitedIsLeaf{ID}} \NC\NR
\NC depth \NC 节点深度（0=根节点） \NC \type{\UnlimitedGetDepth} \NC\NR
\HL
\stoptabulate
...
\startitemize[packed]
\item \type{\UnlimitedDisplayPoint{ID}} - 获取节点分数
\item \type{\UnlimitedDisplayAnswer{ID}} - 获取节点答案
\item \type{\UnlimitedTotalPoint{ID}} - 获取节点总分（包含子题）
\item \type{\UnlimitedTotalNumber{ID}} - 获取节点总数（包含子题）
\item \type{\UnlimitedGetOrder{ID}} - 获取节点层级顺序
\item \type{\UnlimitedIsLeaf{ID}} - 判断是否为叶子节点
\stopitemize
...
\section{Structure 层级统计}

模块提供了按章节层级统计题目的功能，可以获取指定层级的题目数量和分数信息：

\starttabulate[|l|p(8cm)|]
\HL
\NC 命令 \NC 说明 \NC\NR
\HL
\NC \type{\UnlimitedStructureTotalNumber{level}{number}} \NC 指定层级的顶层题目数量（大题数） \NC\NR
\NC \type{\UnlimitedStructureTotalPoint{level}{number}} \NC 指定层级的总分 \NC\NR
\NC \type{\UnlimitedStructureLeafNumber{level}{number}} \NC 指定层级的顶层叶子节点数（独立大题数） \NC\NR
\NC \type{\UnlimitedStructureAllLeafNumber{level}{number}} \NC 指定层级的所有叶子节点数（实际小题数） \NC\NR
\NC \type{\UnlimitedCurrentStructureTotalNumber{level}} \NC 当前层级的顶层题目数量 \NC\NR
\NC \type{\UnlimitedCurrentStructureTotalPoint{level}} \NC 当前层级的总分 \NC\NR
\NC \type{\UnlimitedCurrentStructureLeafNumber{level}} \NC 当前层级的顶层叶子节点数 \NC\NR
\NC \type{\UnlimitedCurrentStructureAllLeafNumber{level}} \NC 当前层级的所有叶子节点数 \NC\NR
\HL
\stoptabulate
...
\startitemize[packed]
\item \type{level}: 层级编号，1=chapter, 2=section, 3=subsection...
\item \type{number}: 该层级的编号（如第1章、第2章）
\stopitemize

{\bf 重要提示}：使用 \type{\UnlimitedCurrentStructure*} 命令时，必须在对应的章节/节结构内调用，否则会返回 0。

\subsection{统计概念说明}

\startitemize[packed]
\item {\bf 顶层题目（Major questions）}: 直接位于章节下的大题，不包含在其他题目内
\item {\bf 顶层叶子节点（Independent major questions）}: 没有子题的顶层大题，即独立大题
\item {\bf 所有叶子节点（All sub-questions）}: 所有实际需要作答的小题，包括嵌套的子题
\stopitemize
...
统计结果：
\startitemize[packed]
\item 顶层题目数：3（大题1、大题2、大题3）
\item 顶层叶子节点数：1（只有大题2是独立大题）
\item 所有叶子节点数：6（小题1.1、1.2、大题2、小题3.1、3.2、3.3）
\stopitemize

\section{题目层级统计}

除了章节级别的统计，模块还提供了题目级别的层级统计功能：

\starttabulate[|l|p(8cm)|]
\HL
\NC 命令 \NC 说明 \NC\NR
\HL
\NC \type{\UnlimitedQuestionLevelNumber{ID}{depth}} \NC 获取指定题目的某一层级的子题目数 \NC\NR
\NC \type{\UnlimitedQuestionLeafNumber{ID}} \NC 获取指定题目的所有叶子节点数（小题数） \NC\NR
\HL
\stoptabulate
...
\startitemize[packed]
\item \type{ID}: 题目节点ID（如 \type{jobname-1-Q-1}）
\item \type{depth}: 相对层级深度（1=第一层子题，2=第二层子题...）
\stopitemize

\section{额外数据}

通过 \type{\UnlimitedSetExtraData{key}{value}} 可以设置额外的节点信息，这些信息可以通过 \type{\UnlimitedGetCurrentExtraData{key}} 获取。
...
\NC 键名 \NC 说明 \NC\NR
\HL
\NC totalitemnumber \NC choice 环境的选项数量 \NC\NR
\NC availablecolumns \NC 可用列数 \NC\NR
\NC itemmaxwidth \NC 选项最大宽度 \NC\NR
\HL
\stoptabulate
...
第一章叶子节点数: \UnlimitedStructureLeafNumber{1}{1}
\stopbuffer
...
题目1总分: \UnlimitedGetAnswer{jobname-1-Q-1}{totalpoint}
题目1总数: \UnlimitedGetAnswer{jobname-1-Q-1}{totalnumber}
\stopbuffer

\typebuffer

\subsection{在题目内部获取信息}

\startbuffer
\question[point=5,answer=A]{题目
  当前ID: \UnlimitedCurrentID
  当前深度: \UnlimitedGetDepth
  当前答案: \UnlimitedDisplayAnswer{\UnlimitedCurrentID}
  当前分数: \UnlimitedDisplayPoint{\UnlimitedCurrentID}
}
\stopbuffer
